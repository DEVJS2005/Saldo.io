  ...
  const addTransaction = async (data) => {
    ...
    // HANDLE INSTALLMENTS
      ...
      for (let i = 0; i < loopCount; i++) {
         ...
         // Explicitly clone startDate and ensure i is treated as offset
    ...
    if (user.canSync) {
        ...
        // Local Mode (Dexie) - Map snake_case to camelCase
      ...
      if (user.canSync) {
          ...
          if (typeof data.isRecurring === 'boolean') {
              ...
              if (data.isRecurring && !data.recurrenceId) {
                 ...
                 if (transactionsToAdd.length > 0) {
                     await supabase.from('transactions').insert(transactionsToAdd);
                 }
              }
          }
          ...
          if (data.currentInstallment !== undefined) updateData.installmentNumber = data.currentInstallment;
          if (data.installments !== undefined) updateData.totalInstallments = data.installments;
